	...
	foreach ($lines as $line)
	{
		...
		if (preg_match("/<([a-zA-Z ]+)/", $line, $regs) > 0)
		{
			...
			if ($tag == "pubDate")
			{
				...
				$values['pubDate'] = strftime($I18N_timeFormat,strtotime($values['pubDate']));
...
{
	MSG_show($message, $language, $width, 'newfeaturetable', 'I18N_warning');
};
...
**parameter language: two character language description (e.g. de, en, fr,...)
**parameter width: width of the box
**parameter urgency: type of message (e.g. errortable, newfeaturetable, infotable)
**/
